...
function Sprite(cfg){
	...
	this.size+=config.randomInt(-2,2);
}
...
Sprite.prototype={
	...
	ty : null,	
	findTarget : function(){
		var nodes=config.background.nodes;

		var index=config.randomInt(0,nodes.length-1);
		var node=nodes[index];
		this.tx=node.x+config.randomInt(-2,2);
		this.ty=node.y+config.randomInt(-2,2);
		node.f++;
	...
	explode : function(){
		this.ax=0;
		this.ay=0;
		this.vx=config.randomInt(-2,2)/10;
		this.vy=config.randomInt(-5,-3)/10;
		this.exploded=true;
	},
